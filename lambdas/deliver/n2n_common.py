...
def save_raw_event(table_name: str, device_sn: str, timestamp_iso: str, payload: dict) -> None:
    ...
    try:
        ...
        table.put_item(Item={
            ...
            "payload": floats_to_decimal(payload),
            "ttl": ttl,
        })
...
def build_reading_list(dm_payload: dict) -> list[dict]:
    ...
    # ── Position (only if present — not all transmissions include GPS) ─────────
    if "lat" in dm_payload and "lng" in dm_payload:
        add("latitude",    "analog",    dm_payload["lat"],                   "deg")
        add("longitude",   "analog",    dm_payload["lng"],                   "deg")
        ...
        add("pos-accuracy","analog",    dm_payload.get("posAcc", 0),         "m")

    # ── Analogues — raw passthrough ──────────────────────────────────────────
    for ch in dm_payload.get("analogues", []):
        add(f"analogue-{ch['id']}", "analog", ch["val"])

    # ── Counters — raw passthrough ───────────────────────────────────────────
    for ct in dm_payload.get("counters", []):
        add(f"counter-{ct['id']}", "analog", ct["val"])
